...
export async function onboardQuotationAsCustomer({ quotationId }) {
  await ensureQuotationTables();
  ...
  const updated = await query(
    `UPDATE quotation_requests SET status='CONVERTED_TO_CUSTOMER', converted_customer_id=$1, updated_at=NOW() WHERE id=$2 RETURNING *`,
    [customer.id, quotationId]
  );

  return { customer, quotation: normalizeQuotationRow(updated.rows[0]) };
}

export async function onboardQuotationAsProject({ quotationId, agreedAmount, advanceAmount }) {
  await ensureQuotationTables();
  ...
  return withTransaction(async () => {
    ...
    const quotation = existing.rows[0];
    if (!quotation) throw new Error("Quotation not found.");
    if (quotation.status === "CONVERTED_TO_PROJECT") throw new Error("This quotation has already been onboarded as a project.");
    ...
    const updated = await query(
      `UPDATE quotation_requests SET status='CONVERTED_TO_PROJECT', converted_customer_id=$1, updated_at=NOW() WHERE id=$2 RETURNING *`,
...
export async function listOngoingProjects({ actor, page = 1, pageSize = 25, search = "" }) {
  ...
  return { rows: rows.rows.map(normalizeProjectRow), total: count.rows[0]?.total || 0, page: safePage, pageSize: safePageSize };
}
