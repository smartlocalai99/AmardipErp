...
export async function onboardQuotationAsProject({ quotationId, agreedAmount, advanceAmount, actor }) {
  ...
  return withTransaction(async () => {
    ...
    if (!customerId) {
      ...
      // AMC dates. The real 1-year AMC window gets set once the
      // installation checklist actually reaches 100% (see
      // markProjectFullyComplete below), not fabricated up front.
      const customerResult = await query(
        `
        INSERT INTO elevator_service_customers (
          customer_code, customer_name, address, mobile_no,
        ...
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'ON GOING',$9)
        RETURNING id
        `,
        [quotation.quotation_no, quotation.customer_name, quotation.address, quotation.mobile_no, quotation.no_of_floors, String(quotation.no_of_passenger), quotation.door_type, quotation.cabin_type, `Onboarded from project ${quotation.quotation_no}. AMC starts once installation is complete.`]
      );
