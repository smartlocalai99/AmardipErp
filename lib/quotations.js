...
import { formatCustomerDateDDMMYYYY } from "./customerDates.js";
...
export function validateProjectAmounts(input = {}) {
  ...
  // Advance is optional — a project can be onboarded with nothing paid up
  // front yet, unlike agreed amount which always has to be a real figure.
  const rawAdvance = input.advanceAmount ?? input.advance_amount;
  const advanceAmount = rawAdvance === undefined || rawAdvance === null || rawAdvance === "" ? 0 : Number(rawAdvance);
  if (!Number.isFinite(advanceAmount) || advanceAmount < 0) throw new Error("Advance amount must be zero or greater.");
...
export async function ensureQuotationTables() {
  ...
  await query(`
    ...
    -- Legacy "ONGOING" sheet rows predate the quotation-onboarding flow —
    -- adopting one into a real, manageable project record has no
    -- originating quotation to point at.
    ALTER TABLE quotation_projects ALTER COLUMN quotation_id DROP NOT NULL;
  `);
...
export async function onboardQuotationAsCustomer({ quotationId, actor }) {
  ...
  const customerResult = await query(
    ...
    [
      ...
      quotation.address,
      // Quotations only ever collect one free-text location field, and
      // staff use it to mean the city ("Kadapa") in practice — so it's
      // stored in both columns rather than leaving city permanently blank
      // for every onboarded customer.
      quotation.address,
      quotation.mobile_no,
...
export async function onboardQuotationAsProject({ quotationId, agreedAmount, advanceAmount, actor }) {
  ...
  return withTransaction(async () => {
    ...
    if (!customerId) {
      // Unlike the plain customer-onboarding path (an existing working lift
      // going straight onto a contract), a project hasn't been installed
      // yet — there's no real AMC to start. Matches every other
      // in-progress installation in the master sheet: ON GOING status, no
      // AMC dates. The real handover (WARRANTY) happens once the
      // installation checklist actually reaches 100% (see
      // markProjectHandedOver), not fabricated up front.
      const customerResult = await query(
        `
        INSERT INTO elevator_service_customers (
          customer_code, customer_name, address, city, mobile_no,
          no_of_floors, no_of_passenger, door_type, cabin,
          customer_status, remarks
        )
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'ON GOING',$10)
        RETURNING id
        `,
        [quotation.quotation_no, quotation.customer_name, quotation.address, quotation.address, quotation.mobile_no, quotation.no_of_floors, String(quotation.no_of_passenger), quotation.door_type, quotation.cabin_type, `Onboarded from project ${quotation.quotation_no}. AMC starts once installation is complete.`]
      );
...
// Legacy "ONGOING" sheet rows predate the quotation-onboarding flow — no
// quotation, no BOQ, just a name/address/mobile the office already tracks
// by hand in the sheet. Adopting one creates a real quotation_projects row
// (quotation_id left NULL — there was never a quotation) so it gets the
// same Start/checklist/edit support as a quotation-onboarded project.
// Most of these customers already exist in elevator_service_customers with
// customer_status='ON GOING' from an earlier bulk import (often missing a
// mobile number), so matching by name within that status is far more
// reliable here than matching by mobile or a customer_code — the sheet's
// own SNO was never written into either column for these older rows.
export async function adoptLegacySheetProject({ customerName, mobileNo, address, city, agreedAmount, advanceAmount, googleSheetRow, actor }) {
  await ensureQuotationTables();
  const name = requiredText(customerName, "Customer name");
  const amounts = validateProjectAmounts({ agreedAmount, advanceAmount });
  const cleanMobile = textOrNull(mobileNo);
  const cleanAddress = textOrNull(address);
  const cleanCity = textOrNull(city);

  return withTransaction(async () => {
    const matched = await query(
      `SELECT id FROM elevator_service_customers
       WHERE customer_status = 'ON GOING' AND LOWER(TRIM(customer_name)) = LOWER(TRIM($1))
       LIMIT 1`,
      [name]
    );
    let customerId = matched.rows[0]?.id || null;

    if (customerId) {
      // Backfill whatever the sheet has that the DB record is missing
      // (commonly the mobile number) without clobbering real data already
      // on file with blanks.
      await query(
        `UPDATE elevator_service_customers
         SET mobile_no = COALESCE(NULLIF(mobile_no, ''), $1),
             address = COALESCE(NULLIF(address, ''), $2),
             city = COALESCE(NULLIF(city, ''), $3),
             updated_at = NOW()
         WHERE id = $4`,
        [cleanMobile, cleanAddress, cleanCity, customerId]
      );
    } else {
      const inserted = await query(
        `INSERT INTO elevator_service_customers (customer_name, address, city, mobile_no, customer_status, remarks)
         VALUES ($1, $2, $3, $4, 'ON GOING', 'Adopted from ONGOING sheet.')
         RETURNING id`,
        [name, cleanAddress, cleanCity, cleanMobile]
      );
      customerId = inserted.rows[0].id;
    }

    const existingProject = await query("SELECT id FROM quotation_projects WHERE customer_id = $1 LIMIT 1", [customerId]);
    if (existingProject.rows[0]) throw new Error("This project has already been adopted.");

    const projectResult = await query(
      `INSERT INTO quotation_projects (quotation_id, customer_id, agreed_amount, advance_amount, balance_amount, google_sheet_row)
       VALUES (NULL, $1, $2, $3, $4, $5)
       RETURNING *`,
      [customerId, amounts.agreedAmount, amounts.advanceAmount, amounts.balanceAmount, googleSheetRow || null]
    );
    const customerRow = await query("SELECT customer_name, mobile_no, address, city FROM elevator_service_customers WHERE id = $1", [customerId]);
    return normalizeProjectRow({ ...projectResult.rows[0], ...customerRow.rows[0] });
  });
}

// Edits an already-real project's customer details and/or contract amounts
// — used both for quotation-onboarded projects and for ones just adopted
// from the legacy sheet, since both are plain quotation_projects rows by
// this point. Best-effort sync back to the ONGOING sheet is the caller's
// job (this only touches Postgres) since a Sheets hiccup shouldn't fail an
// edit already committed to the database.
export async function updateProjectDetails({ projectId, customerName, mobileNo, address, city, agreedAmount, advanceAmount, actor }) {
  await ensureQuotationTables();
  const amounts = validateProjectAmounts({ agreedAmount, advanceAmount });
  const name = requiredText(customerName, "Customer name");

  return withTransaction(async () => {
    const existing = await query("SELECT * FROM quotation_projects WHERE id = $1 FOR UPDATE", [projectId]);
    const project = existing.rows[0];
    if (!project) throw new Error("Project not found.");

    await query(
      `UPDATE elevator_service_customers
       SET customer_name = $1, mobile_no = $2, address = $3, city = $4, updated_at = NOW()
       WHERE id = $5`,
      [name, textOrNull(mobileNo), textOrNull(address), textOrNull(city), project.customer_id]
    );
    const updatedProject = await query(
      `UPDATE quotation_projects
       SET agreed_amount = $1, advance_amount = $2, balance_amount = $3, updated_at = NOW()
       WHERE id = $4
       RETURNING *`,
      [amounts.agreedAmount, amounts.advanceAmount, amounts.balanceAmount, projectId]
    );
    return normalizeProjectRow({ ...updatedProject.rows[0], customer_name: name, mobile_no: textOrNull(mobileNo), address: textOrNull(address), city: textOrNull(city) });
  });
}

// Called once a project's installation checklist reaches 100% — this is
// the actual handover (HOC) moment. Every real customer in this system
// gets one free year of WARRANTY starting from handover before any paid
// AMC begins (see lib/customerDates.js's IN_WARRANTY_SQL, which computes
// "still under warranty" live from hoc_date) — a completed installation
// does NOT jump straight to a paid AMC contract. Idempotent: re-running
// this once hoc_date is already set is a no-op, so a technician
// re-checking an already-complete step can't reset the handover date.
export async function markProjectHandedOver(customerId) {
  const existing = await query("SELECT hoc_date FROM elevator_service_customers WHERE id = $1 LIMIT 1", [customerId]);
  if (!existing.rows[0] || existing.rows[0].hoc_date) return null;

  const today = new Date();
  const hocDate = formatCustomerDateDDMMYYYY(today);
  const warrantyDue = formatCustomerDateDDMMYYYY(new Date(today.getFullYear() + 1, today.getMonth(), today.getDate()));

  const result = await query(
    `UPDATE elevator_service_customers
     SET customer_status = 'WARRANTY', hoc_date = $1, amc_warranty_due = $2,
         amc_starting_date = NULL, amc_ending_date = NULL, updated_at = NOW()
     WHERE id = $3
     RETURNING *`,
    [hocDate, warrantyDue, customerId]
  );
...
export async function listOngoingProjects({ actor, page = 1, pageSize = 25, search = "" }) {
  ...
  const count = await query(`SELECT COUNT(*)::int AS total FROM quotation_projects p LEFT JOIN quotation_requests q ON q.id = p.quotation_id JOIN elevator_service_customers c ON c.id = p.customer_id ${where}`, params);
  const safePage = Math.max(1, Number(page) || 1);
  const safePageSize = Math.min(100, Math.max(1, Number(pageSize) || 25));
  const rows = await query(
    `SELECT p.*, q.quotation_no, c.customer_name, c.mobile_no, c.address, c.city
     FROM quotation_projects p
     LEFT JOIN quotation_requests q ON q.id = p.quotation_id
...
// Just enough to log one Erection Sheet row (customer, city, crew names) —
// one query with the crew pulled in via a correlated subquery instead of a
// second round trip, since this only runs when a step is actually marked
// done, not on every checklist read.
export async function getProjectSheetLogSummary(projectId) {
  const result = await query(
    `SELECT c.id AS customer_id, c.customer_name, c.city,
  ...
  if (!row) return null;
  return { customerId: row.customer_id, customerName: row.customer_name, city: row.city, crewNames: row.crew_names || [] };
}
