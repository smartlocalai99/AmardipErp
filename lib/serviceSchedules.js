...
let serviceSchedulesTableReady = false;

export async function ensureServiceSchedulesTable() {
  if (serviceSchedulesTableReady) return;
  ...
  // No FK to complaints(id) — service_schedules can be set up before the
  // complaints table exists; the app treats this as an informational link.
  await query(`ALTER TABLE service_schedules ADD COLUMN IF NOT EXISTS linked_complaint_id UUID`);
  ...
  `);
  serviceSchedulesTableReady = true;
}
