import { query, withTransaction } from "./db.js";
import { ensureInventoryTables, getInventoryItemById } from "./inventory.js";
import { ensureComplaintsTable, getComplaintById } from "./complaints.js";
import { verifyJobPass } from "./jobPass.js";

let tableReady = false;

export async function ensureMaterialRequestsTable() {
  if (tableReady) return;
  await ensureInventoryTables();
  await ensureComplaintsTable();
  tableReady = true;
}

export function normalizeRequestRow(row = {}) {
  if (!row) return null;
  return {
    id: row.id,
    complaintId: row.complaint_id,
    itemId: row.item_id,
    itemName: row.item_name || null,
    itemUnit: row.item_unit || null,
    requestedBy: row.requested_by,
    requestedByName: row.requested_by_name || null,
    complaintNo: row.complaint_no || null,
    customerName: row.customer_name || null,
    assignedTechnicianName: row.assigned_technician_name || null,
    requestedQuantity: Number(row.requested_quantity),
    issuedQuantity: Number(row.issued_quantity),
    status: row.status,
    approvedBy: row.approved_by || null,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
}

const SELECT_WITH_JOINS = `
  SELECT r.*, i.name AS item_name, i.unit AS item_unit, u.name AS requested_by_name,
         c.complaint_no, c.customer_name, c.assigned_technician_name
  FROM material_requests r
  JOIN inventory_items i ON i.id = r.item_id
  LEFT JOIN users u ON u.id = r.requested_by
  JOIN complaints c ON c.id = r.complaint_id
`;

export function normalizeAllocatedItems(raw) {
  if (!Array.isArray(raw)) return [];
  return raw
    .map((line) => ({ itemId: Number(line?.itemId), quantity: Number(line?.quantity) }))
    .filter((line) => line.itemId && line.quantity > 0);
}

export async function createMaterialRequests({ complaintId, requestedBy, items, status = "pending" }) {
  await ensureMaterialRequestsTable();
  if (!Array.isArray(items) || items.length === 0) throw new Error("At least one item is required.");
  if (!["pending", "approved"].includes(status)) throw new Error("Invalid status.");

  const complaint = await getComplaintById(complaintId);
  if (!complaint) throw new Error("Job not found.");

  const created = [];
  for (const line of items) {
    const item = await getInventoryItemById(line.itemId);
    if (!item) throw new Error(`Inventory item ${line.itemId} not found.`);
    const quantity = Number(line.quantity);
    if (!quantity || quantity <= 0) throw new Error(`Invalid quantity for ${item.name}.`);

    const result = await query(
      `
      INSERT INTO material_requests (complaint_id, item_id, requested_by, requested_quantity, status)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
      `,
      [complaintId, item.id, requestedBy, quantity, status]
    );
    created.push(normalizeRequestRow({ ...result.rows[0], item_name: item.name, item_unit: item.unit }));
  }

  return created;
}

export async function listAllMaterialRequests({ status } = {}) {
  await ensureMaterialRequestsTable();
  const params = [];
  let whereSql = "";
  if (status) {
    params.push(status);
    whereSql = "WHERE r.status = $1";
  }
  const result = await query(
    `${SELECT_WITH_JOINS} ${whereSql} ORDER BY r.created_at DESC LIMIT 500`,
    params
  );
  return result.rows.map(normalizeRequestRow);
}

export async function listMaterialRequestsForComplaint(complaintId) {
  await ensureMaterialRequestsTable();
  const result = await query(
    `${SELECT_WITH_JOINS} WHERE r.complaint_id = $1 ORDER BY r.created_at DESC`,
    [complaintId]
  );
  return result.rows.map(normalizeRequestRow);
}

function assertJobOpenForWorker(complaint, workerId) {
  if (!complaint) throw new Error("Job not found.");
  // A job can have a junior technician assigned alongside the primary —
  // checking only assignedTechnicianUserId (the primary) meant a junior's
  // own generated store pass always got rejected here, even though
  // canViewComplaint (used to let them generate the pass in the first
  // place) already treats every assignee as valid for the job.
  const isAssignee = Number(complaint.assignedTechnicianUserId) === Number(workerId)
    || (complaint.assignees || []).some((assignee) => Number(assignee.id) === Number(workerId));
  if (!isAssignee) {
    throw new Error("This job pass is no longer valid for this worker.");
  }
  // No longer rejected once the job is resolved/closed — the same pass is
  // what the store scans afterward to collect back unused parts, so it
  // has to keep resolving after completion too.
}

export async function resolveJobPass(token) {
  const { complaintId, workerId } = verifyJobPass(token);
  const complaint = await getComplaintById(complaintId);
  assertJobOpenForWorker(complaint, workerId);
  return { complaint, workerId };
}

// Stock, ledger, and requests commit together on a reserved connection.
export async function issueMaterialsForJob({ token, items, actor }) {
  await ensureMaterialRequestsTable();
  if (!Array.isArray(items) || items.length === 0) throw new Error("At least one item is required.");

  const { complaint, workerId } = await resolveJobPass(token);

  return withTransaction(async () => {
    const issued = [];
    for (const line of items) {
      const quantity = Number(line.quantity);
      if (!quantity || quantity <= 0) throw new Error("Invalid quantity.");

      const itemResult = await query("SELECT * FROM inventory_items WHERE id = $1 FOR UPDATE", [line.itemId]);
      const item = itemResult.rows[0];
      if (!item) throw new Error(`Inventory item ${line.itemId} not found.`);
      const currentStock = Number(item.stock_quantity);
      if (currentStock < quantity) {
        throw new Error(`Insufficient stock for ${item.name}: only ${currentStock} ${item.unit} available.`);
      }

      const newStock = currentStock - quantity;
      await query("UPDATE inventory_items SET stock_quantity = $1, updated_at = NOW() WHERE id = $2", [newStock, item.id]);
      await query(
        `
        INSERT INTO inventory_transactions (item_id, complaint_id, type, quantity_delta, balance_after, performed_by, worker_id, notes)
        VALUES ($1, $2, 'issue', $3, $4, $5, $6, $7)
        `,
        [item.id, complaint.id, -quantity, newStock, actor?.id || null, workerId, line.notes || null]
      );

      const matchingRequest = await query(
        "SELECT id FROM material_requests WHERE complaint_id = $1 AND item_id = $2 AND status IN ('pending', 'approved', 'partially_issued') LIMIT 1",
        [complaint.id, item.id]
      );
      if (matchingRequest.rows[0]) {
        await query(
          "UPDATE material_requests SET issued_quantity = issued_quantity + $1, status = 'issued', updated_at = NOW() WHERE id = $2",
          [quantity, matchingRequest.rows[0].id]
        );
      } else {
        await query(
          `
          INSERT INTO material_requests (complaint_id, item_id, requested_by, requested_quantity, issued_quantity, status, approved_by)
          VALUES ($1, $2, $3, $4, $4, 'issued', $3)
          `,
          [complaint.id, item.id, workerId, quantity]
        );
      }

      issued.push({ itemId: item.id, name: item.name, unit: item.unit, quantity, balanceAfter: newStock });
    }

    return { complaint, issued, workerId };
  });
}

// The QR flow (issueMaterialsForJob above) is one way to hand parts to a
// worker; this is the other — a worker who already knows what a job needs
// (or one they didn't pick up in person) requests it from their own app,
// and the store issues straight from that request, no QR involved. Same
// stock/ledger effect either way, just triggered from a request row
// instead of a scanned pass.
export async function issueMaterialRequestDirectly({ requestId, quantity, actor }) {
  await ensureMaterialRequestsTable();

  return withTransaction(async () => {
    const requestResult = await query(
      `SELECT r.*, i.name AS item_name, i.unit AS item_unit
         FROM material_requests r
         JOIN inventory_items i ON i.id = r.item_id
        WHERE r.id = $1
        FOR UPDATE`,
      [requestId]
    );
    const request = requestResult.rows[0];
    if (!request) throw new Error("Request not found.");
    if (["issued", "rejected"].includes(request.status)) {
      throw new Error("This request has already been closed out.");
    }

    const remaining = Number(request.requested_quantity) - Number(request.issued_quantity);
    const issueQuantity = quantity === undefined || quantity === null || quantity === ""
      ? remaining
      : Number(quantity);
    if (!Number.isFinite(issueQuantity) || issueQuantity <= 0) {
      throw new Error("Enter a valid quantity to issue.");
    }
    if (issueQuantity > remaining) {
      throw new Error(`Only ${remaining} ${request.item_unit} still outstanding on this request.`);
    }

    const itemResult = await query("SELECT * FROM inventory_items WHERE id = $1 FOR UPDATE", [request.item_id]);
    const item = itemResult.rows[0];
    const currentStock = Number(item.stock_quantity);
    if (currentStock < issueQuantity) {
      throw new Error(`Insufficient stock for ${item.name}: only ${currentStock} ${item.unit} available.`);
    }

    const newStock = currentStock - issueQuantity;
    await query("UPDATE inventory_items SET stock_quantity = $1, updated_at = NOW() WHERE id = $2", [newStock, item.id]);
    await query(
      `INSERT INTO inventory_transactions (item_id, complaint_id, type, quantity_delta, balance_after, performed_by, worker_id)
       VALUES ($1, $2, 'issue', $3, $4, $5, $6)`,
      [item.id, request.complaint_id, -issueQuantity, newStock, actor?.id || null, request.requested_by]
    );

    const newIssuedQuantity = Number(request.issued_quantity) + issueQuantity;
    const newStatus = newIssuedQuantity >= Number(request.requested_quantity) ? "issued" : "partially_issued";
    await query(
      `UPDATE material_requests
          SET issued_quantity = $1, status = $2, approved_by = COALESCE(approved_by, $3), updated_at = NOW()
        WHERE id = $4`,
      [newIssuedQuantity, newStatus, actor?.id || null, requestId]
    );

    return {
      requestId,
      itemId: item.id,
      name: item.name,
      unit: item.unit,
      issuedQuantity: issueQuantity,
      status: newStatus,
      balanceAfter: newStock,
      workerId: request.requested_by,
      complaintId: request.complaint_id,
    };
  });
}
