...
export async function listComplaints({ actor, filters = {}, page = 1, pageSize = 25, workerOnly = false, customerOnly = false }) {
  ...
  // Customers and the worker themselves both need to see everything that
  // was recorded once a job is done — not just that it's "Resolved" — so
  // attach the job-completion record (checklist, notes, GPS) either way.
  // Otherwise reopening an already-completed job (or one another assignee
  // already worked) shows a blank form instead of what was actually saved.
  if (customerOnly || workerOnly) {
  ...
  try {
    const result = await query(
      `SELECT DISTINCT ON (complaint_id)
         complaint_id, problem_identified, work_performed, spare_parts_used,
         status_resolution, checklist_data, customer_rep_name,
         gps_latitude, gps_longitude, gps_accuracy_meters, completed_at
       FROM technician_job_completions
       WHERE complaint_id = ANY($1)
       ORDER BY complaint_id, completed_at DESC`,
      [ids]
    );
    const map = new Map();
...
export async function getComplaintById(id) {
  ...
  if (complaint) complaint.assignees = await getComplaintAssignees(complaint.id);
  return complaint;
}
