...
export async function listComplaints({ actor, filters = {}, page = 1, pageSize = 25, workerOnly = false, customerOnly = false }) {
  ...
  // Customers need to see everything the technician recorded once a job is
  // done — not just that it's "Resolved" — so attach the job-completion
  // record (checklist, notes, GPS) for their own view.
  if (customerOnly) {
