...
export function buildComplaintWhere({ actor, filters = {}, workerOnly = false, customerOnly = false }) {
  ...
  if (customerOnly) {
    ...
    // dispatches, not a customer-raised breakdown — they belong in the
    // Service tab (elevator_service_visits), not the Tickets list.
    clauses.push(`complaint_type != 'SERVICE_REQUEST'`);
  }
