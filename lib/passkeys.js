...
export const ensurePasskeyTables = createSchemaInitializer(async () => {
  await query(`
  ...
  // credential_id and flow_id already have indexes from their UNIQUE constraints.
...
export async function getUserPasskeyDevices(userId) {
  const result = await query(
    `
      SELECT
        id,
        COALESCE(NULLIF(TRIM(device_name), ''), 'Face Lock Device') AS device_name,
        created_at,
        last_used_at
      FROM user_passkeys
      WHERE user_id = $1
...
export async function removeUserPasskeyDevice(userId, id) {
  const result = await query(
    `
