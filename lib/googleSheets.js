...
// Writes one converted project (or, via appendOnboardedCustomerRow below,
// one directly-onboarded AMC customer) to the existing ongoing-projects tab.
// The tab is resolved by its stable gid, while the row number is persisted by
// the caller so a retry cannot append the same project twice.
export async function appendOngoingProjectRow(project, quotation) {
  ...
  // values.append's own "find the table, then append below it" heuristic
  // has silently misaligned rows on this sheet before — it detected a
  // table starting at column E instead of A (probably thrown off by a
  // stray value further right somewhere below), so every field landed one
  // column group to the right of where it should. Finding the real next
  // empty row ourselves off column A (always populated with the SNO) and
  // writing with an explicit, anchored A:F range sidesteps that heuristic
  // entirely instead of trusting append() to guess correctly.
  const colA = await sheets.spreadsheets.values.get({ spreadsheetId, range: `'${tabTitle}'!A:A` });
  const nextRow = (colA.data.values?.length || 0) + 1;

  await sheets.spreadsheets.values.update({
    spreadsheetId,
    range: `'${tabTitle}'!A${nextRow}:F${nextRow}`,
    valueInputOption: "USER_ENTERED",
    requestBody: {
      values: [[
...
// Mirrors an edited project's customer details back onto its own row in the
// ONGOING tab (B:E — name/address/city/mobile; column A's SNO and column
// F's status are left alone) so the sheet doesn't drift from what admin
// just corrected in the app. Best-effort by design — callers already treat
// a Sheets failure as non-fatal, same as every other sheet sync here.
export async function updateOngoingSheetRow(rowNumber, { customerName, address, city, mobileNo }) {
  if (!rowNumber) return;
  const sheets = getSheetsClient();
  const spreadsheetId = process.env.GOOGLE_ONGOING_PROJECT_SHEET_ID || DEFAULT_ONGOING_PROJECT_SHEET_ID;
  const tabTitle = await getOngoingProjectsTabTitle();
  await sheets.spreadsheets.values.update({
    spreadsheetId,
    range: `'${tabTitle}'!B${rowNumber}:E${rowNumber}`,
    valueInputOption: "USER_ENTERED",
    requestBody: {
      values: [[customerName || "", address || "", city || "", mobileNo || ""]],
    },
  });
}
