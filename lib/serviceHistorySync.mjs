...
export function parseServiceHistoryRows(values) {
  ...
  dataRows.forEach((row, index) => {
    ...
    // Rows the app itself wrote (see lib/serviceHistorySheetWriter.js) are
    // already inserted directly into elevator_service_visits at job
    // completion, under a different source_sheet key. Re-importing them
    // here too — matched only by this sheet's own row number — would
    // create a second, duplicate row in the DB for the same real visit.
    const sourceMarker = cleanText(getCell(row, columnMap, "SOURCE"));
    if (sourceMarker && sourceMarker.trim().toUpperCase() === "APP") return;
...
export async function fetchServiceHistoryRows() {
  ...
  const response = await sheets.spreadsheets.values.get({
    ...
    range: `'${SERVICE_HISTORY_SOURCE_TAB}'!A:AB`,
...
export async function syncServiceHistory({ apply = true } = {}) {
  ...
  try {
    const matchingData = await loadCustomerMatchingData(client);
