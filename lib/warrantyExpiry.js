...
import { createSchemaInitializer } from "./schemaInit.js";
...
export const ensureWarrantyExpiryNoticesSchema = createSchemaInitializer(async () => {
  await query(`
  ...
  // The exact HOC/warranty-due date text shown on the sent letter, fetched
  // from the source sheet at send time (see fetchSheetRowForCustomer) and
  // fixed here so the document a customer later re-opens always matches
  // what was actually sent, not whatever the sheet says today.
  await query("ALTER TABLE warranty_expiry_notices ADD COLUMN IF NOT EXISTS hoc_date_text VARCHAR(20)");
  await query("ALTER TABLE warranty_expiry_notices ADD COLUMN IF NOT EXISTS warranty_due_date_text VARCHAR(20)");
});

// Warranty customers (per the live HOC-date rule) whose warranty ends within
// `daysAhead` days — this is what the admin sees to decide who to send a
// letter to. Customers already sent a letter stay in this list (with
// sentAt/amcAmount populated) so the admin can see who's been handled,
// rather than vanishing once sent. Sending is manual and per-customer (an
// admin-entered AMC amount is required each time), not an automated job.
...
// Looks up one customer's row in the staff-maintained source sheet: their
// current status there (to catch an already-converted-to-AMC customer the
// local DB hasn't caught up on) and their HOC date (the letter's HOC/
// warranty-due dates are sourced from the sheet, per the admin's request,
// not recomputed from our own DB). Sheet-unreachable/not-configured fails
// open — returns null — so a single send action never gets blocked by an
// unrelated outage; the caller falls back to the DB's own HOC date.
async function fetchSheetRowForCustomer(customerCode) {
  const sheets = getSheetsClient();
  if (!sheets || !customerCode) return null;
  ...
  try {
    ...
    for (const row of rows) {
      ...
      return {
        hocDateText: String(row[5] || "").trim() || null,
        status: String(row[6] || "").trim().toUpperCase(),
      };
    }
    return null;
  } catch (error) {
    console.error("Warranty-expiry sheet lookup failed, falling back to the database HOC date:", error);
    return null;
  }
}
...
// the notice (which is what makes the PDF servable, and shows up as "sent"
// in the admin's expiring list — the customer stays listed, just marked),
// and notifies them.
export async function sendWarrantyExpiryLetterToCustomer(customerId, amcAmount) {
  ...
  const sheetRow = await fetchSheetRowForCustomer(candidate.customer_code);
  if (sheetRow && CONTRACT_STATUSES.has(sheetRow.status)) {
    throw new Error("The source sheet shows this customer has already converted to AMC — skipped.");
  }

  // HOC date comes from the sheet when available, falling back to the DB's
  // own HOC date otherwise; the due date is always exactly one calendar
  // year after that HOC date, never the sheet's separate due-date column.
  const hoc = (sheetRow?.hocDateText && parseCustomerDate(sheetRow.hocDateText)) || parseCustomerDate(candidate.hoc_date);
