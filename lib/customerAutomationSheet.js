...
// path — to the staff's real master sheet, so it's picked up by the
// due-service heuristic above without someone copying it in by hand later.
// `advance` (column M) only applies to the project-onboarding path; blank
// for a plain AMC-customer onboarding.
export async function appendCustomerAutomationRow({
  customerCode,
  customerName,
  address = "",
  city = "",
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: {
      values: [[
        ...
        "",
        status,
        toSheetDate(amcWarrantyDue),
        toSheetDate(amcStartDate),
        toSheetDate(amcEndDate),
        amcAmount,
        advance,
      ]],
    },
  ...
  // Invalidate the read cache so the due-service heuristic sees this
  // customer immediately instead of waiting out the 5-minute TTL.
  cache = null;
  cacheExpiresAt = 0;
}
