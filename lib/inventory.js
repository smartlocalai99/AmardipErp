...
export const ensureInventoryTables = createSchemaInitializer(async () => {
  ...
  // Once upgraded, leave the constraint in place: dropping and adding it at
  // every cold start takes an exclusive table lock and scans the history again.
  const constraint = await query(`
    SELECT EXISTS (
      SELECT 1 FROM pg_constraint
      WHERE conrelid = 'inventory_transactions'::regclass
        AND conname = 'inventory_transactions_type_check'
        AND contype = 'c'
        AND pg_get_constraintdef(oid) LIKE '%''used''%'
    ) AS supports_used
  `);
  if (!constraint.rows[0]?.supports_used) {
    await query(`
...
export async function listStoreJobsWithOutstandingItems({ search = "" } = {}) {
  ...
  const result = await query(
    `WITH movement AS (
     ...
     SELECT c.id, c.complaint_no, c.customer_name, c.assigned_technician_name, c.status,
            ...
            json_agg(
              json_build_object('itemId', i.id, 'name', i.name, 'unit', i.unit, 'quantity', m.outstanding)
              ORDER BY i.name
            ) AS items
     FROM movement m
  ...
  return result.rows.map((row) => ({
    ...
    // Full breakdown right in the list — so a store person can see who's
    // holding what without opening each job into the return form.
    items: (row.items || []).map((item) => ({ ...item, quantity: Number(item.quantity) })),
  }));
}
